...
Offload blocking operations to a subprocess via L<IO::Async::Function>:
  ...
  $scope->{loop}->add($worker);  # Forks the worker process here
...
The number of workers should typically match your CPU core count:
  ...
  # Auto-detect CPU cores and create that many workers
  pagi-server hello.pl --port 5000 --workers auto
...
For HTTP requests, you receive body chunks:

  {
      ...
      more_body => 0,             # 0 = last chunk, 1 = more coming
  }
...
Let's build a complete HTTP request handler that reads the request body and echoes it back.
  ...
  async sub app {
      ...
      while (1) {
          ...
          last unless $event->{more_body};  # Stop when no more chunks
      }
      ...
      await $send->({
          type => 'http.response.body',
          body => $response_body,
          more_body => 0,  # This is the last (and only) chunk
      });
  }
...
=item * Always check C<more_body> to know when the request body is complete

=item * Send C<http.response.start> before sending any body

=item * Set C<more_body =E<gt> 0> on the final body chunk
  ...
  async sub app {
      ...
      for my $i (1..5) {
          await $send->({
              type => 'http.response.body',
              body => "Chunk $i\n",
              more_body => 1,  # More chunks coming
          });
      ...
      await $send->({
          type => 'http.response.body',
          body => "Done!\n",
          more_body => 0,  # This is the last chunk
      });
  }
...
=item * Set C<more_body =E<gt> 1> on all chunks except the last

=item * Set C<more_body =E<gt> 0> on the final chunk
...
The lifespan protocol allows your application to initialize resources on startup and clean them up on shutdown.
...
=head3 Request Paths
  ...
  $scope->{path}       # Decoded UTF-8 string (use this for display/matching)
...
Query strings are raw bytes. You must decode them yourself:
  ...
  async sub app {
      ...
      for my $pair (split /&/, $query) {
          ...
          $key = decode_utf8(uri_decode($key // ''));
          $value = decode_utf8(uri_decode($value // ''));
          $params{$key} = $value;
      }
      ...
      await $send->({
          type => 'http.response.body',
          body => "Hello, $name!",  # WRONG: Should encode to bytes
      });
  }
...
L<PAGI::Request> and L<PAGI::Response> handle UTF-8 encoding/decoding automatically:
  ...
  async sub app {
      ...
      # Response automatically encodes
      $res->status(200);
      $res->content_type('text/plain');
      $res->body("Hello, $name!");  # Automatically encoded to bytes
...
=head3 Errors
  ...
  async sub app {
      ...
      # Internal server error
          ...
          await $res->error(500);  # Message defaults to "Internal Server Error"
      }
      else {
          await $res->text('OK');
      }
...
The C<error()> method sends a JSON response with this structure:
...
L<PAGI::Request> handles multipart file uploads:
  ...
  async sub app {
      ...
      if ($file && !$file->is_empty) {
          ...
          await $res->error(400, 'No file uploaded');
      }
  }
...
Use C<:param> syntax for named parameters in routes:
  ...
  $router->get('/users/:id' => async sub {
      ...
      # Access parameter via $req->param()
      my $id = $req->param('id');
...
Parameters are stored in C<$scope-E<gt>{'pagi.params'}> and accessed via C<$req-E<gt>param($name)>.
...
WebSocket handlers receive C<($self, $ws)>:
  ...
  async sub handle_chat {
      ...
      my $room = $ws->scope->{'pagi.params'}{room};
...
=head3 Common Fields (All Types)
  ...
  $scope->{loop}         # IO::Async::Loop instance
